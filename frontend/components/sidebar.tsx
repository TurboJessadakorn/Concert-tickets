...
function Sidebar({ isAdmin, toggleUserRole }: SidebarProps) {
    ...
    const handleLogout = () => {
        alert("Logout is not available in this demo website. It is intended for demonstration purposes only.");
    };
    ...
    return (
        <>
            ...
            <aside ref={sidebarRef} id="default-sidebar" className={`fixed top-0 left-0 z-40 w-52 h-screen transition-transform -translate-x-full sm:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`} aria-label="Sidebar">
                <div className="h-full px-3 py-4 overflow-y-auto bg-white border-r flex flex-col justify-between">
                    <div>
                        <div className="px-2 py-6">
                            <h1 className="text-3xl font-semibold">{isAdmin ? 'Admin' : 'User'}</h1>
                        </div>
                        <ul className="space-y-2 font-medium text-lg">
                            ...
                            <li>
                                <a href="/" onClick={toggleUserRole} className="flex items-center px-2 py-3 text-gray-900 rounded-lg hover:bg-gray-200 group">
                                    ...
                                    <span className="ms-3">{isAdmin ? 'Switch to User' : 'Switch to Admin'}</span>
                                </a>
                            </li>
                        </ul>
                    </div>
                    <div className="px-2 py-6">
                        <div className="text-xl group flex align-center hover:bg-gray-200 rounded-lg px-2 py-3 cursor-pointer" onClick={handleLogout}>
                            <svg fill="none" className="w-7 h-7 mr-1 transition duration-75 group-hover:text-gray-900" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M17 16L21 12M21 12L17 8M21 12L7 12M13 16V17C13 18.6569 11.6569 20 10 20H6C4.34315 20 3 18.6569 3 17V7C3 5.34315 4.34315 4 6 4H10C11.6569 4 13 5.34315 13 7V8" stroke="#374151" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" /></svg>
                            Logout
                        </div>
                    </div>
                </div>
